...
table.addEventListener("click", (event)=>{
    ...
    if (title) {
        ...
        newRows.sort((a, b)=>{
            const valueFirst = isSorted ? `${b.cells[index].innerHTML}` : `${a.cells[index].innerHTML}`;
            const valueSecond = isSorted ? `${a.cells[index].innerHTML}` : `${b.cells[index].innerHTML}`;
    ...
    if (row && row.rowIndex !== 0 && row.rowIndex !== tbodyRows.length - 1) {
        let countActive = 0;
        for (const r of tbodyRows)if (r.classList.contains("active")) countActive++;
